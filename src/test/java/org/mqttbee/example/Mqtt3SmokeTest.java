...
class Mqtt3SmokeTest {
    ...
    void mqttOverTlsWithClientCert() throws CertificateException, NoSuchAlgorithmException, KeyStoreException, IOException {
        ...
        publishInstance = new Mqtt3ClientExample(server, 8884, true, trustStore, keyStore, KEYSTORE_PASS);
        publishInstance.publish(topic, qos, count).blockingSubscribe();
    }

    @Test
    void mqttOverWebSockets() {
        publishInstance = new Mqtt3ClientExample(server, 8080, false, null, null, null);
        publishInstance.publish(topic, qos, count).blockingSubscribe();
    }

    @Test
    void mqttOverWebSocketsEncrypted() {
        publishInstance = new Mqtt3ClientExample(server, 8081, true, null, null, null);
        publishInstance.publish(topic, qos, count).blockingSubscribe();
    }
}
